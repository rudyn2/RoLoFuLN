...
if __name__ == '__main__':
    ...
    if plot_noise_1:
        for batch_size in batch_sizes:
            ...
            dmi_models_files_1 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_DMILoss_1_*.pt')
            dmi_models_files_1 = sorted(dmi_models_files_1, key=get_noise_rate_from_name)
            ce_models_files_1 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_CrossEntropyLoss_1_*.pt')
            ...
            axs[0].plot(noise_values, dmi_test_acc_1, '^-', linewidth=3, label='LDMI-'+str(batch_size))
            axs[0].plot(noise_values, ce_test_acc_1, '*--', linewidth=3, label='CE-'+str(batch_size))
            axs[0].set(xlabel='Noise amount ($r$)', ylabel='Test Accuracy (%)')
            axs[0].legend()

    if plot_noise_2:
        for batch_size in batch_sizes:
            dmi_models_files_2 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_DMILoss_2_*.pt')
            dmi_models_files_2 = sorted(dmi_models_files_2, key=get_noise_rate_from_name)
            ce_models_files_2 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_CrossEntropyLoss_2_*.pt')
    ...
    if plot_noise_3:
        for batch_size in batch_sizes:
            dmi_models_files_3 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_DMILoss_3_*.pt')
            dmi_models_files_3 = sorted(dmi_models_files_3, key=get_noise_rate_from_name)
            ce_models_files_3 = glob.glob(parent_dir + f'/models_exp2/batch_{batch_size}/model_CNN_CrossEntropyLoss_3_*.pt')
